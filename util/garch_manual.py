...
def _loglik(pars, r, sum=True):
    w, a, b = pars
    sigma2 = _ARMA(w, a, b, r)

    loglik = -np.sum(-np.log(sigma2) - (r**2)/(2*sigma2))  # minus because will minimize

    loglik_vec = -0.5 * (np.log(2 * np.pi) + np.log(sigma2) + r ** 2.0 / sigma2)
    loglik_sum = -np.sum(loglik_vec)
    if sum:
        return loglik_sum
    else:
        return -1 * loglik_vec
...
def fit_sigma(returns, pars = (10, 0.2, 0.5)):
    res = opt.minimize(_loglik, pars, args=(returns), method="SLSQP",
                bounds=((0.0001, None), (0.0001, None), (0.0001, None)),
                options={'disp':False})
    return res


def fit_ARMA_GARCH(data):
    # fit the GARCH with AR term
    garch = arch_model(data, vol='GARCH', p=1, q=1, mean='ARX')
    garch_fit = garch.fit(disp='off')
    arma_garch_vola = garch_fit._volatility
    return garch_fit
...
def analyse_acf(data):
    """ data: list, series, vector of data """
    from statsmodels.graphics.tsaplots import plot_acf, plot_pacf
    # square the dataset
    squared_data = [x**2 for x in data]
    # create acf plot
    fig = plot_acf(np.array(squared_data))
    fig = plot_pacf(np.array(squared_data))
    return fig
