...
class HdCalculator:
    def __init__(self, data, tau_day, date, S0, burnin, path, M=5000,
                 target='Adj.Close', h=0.2, overwrite=False):
        self.data = data
        self.target = target
    ...
    def get_hd(self):
        ...
        if os.path.exists(self.garch_data + self.filename) and (self.overwrite == False):
            ...
            pd.Series(self.S).to_csv(
                join(self.garch_data, self.filename), index=False)
