...
def SVCJ(tau_day, S0, n=10000, myseed=1234):
    # SVCJ parameters
    mu      = 0.042
    r       = mu
    mu_y    = -0.0492
    ...
    w2 = rho * w + np.sqrt(1 - rho ** 2) * np.random.standard_normal([n, m])
    z_v = np.random.exponential(mu_v, [n, m])
    z_y = np.random.standard_normal([n, m]) * sigma_y + mu_y + rho_j * z_v
    dj = np.random.binomial(1, l * dt, size=[n, m])
    s = np.zeros([n, m + 1])
    v = np.zeros([n, m + 1])
