...
def plot_MKM(RndData, HdData, day, tau_day, x, y_lim=None, reset_S=False,
             overwrite=False, h_densfit=0.2):
    ...
    HD = HdCalculator(hd_data, tau_day=tau_day, date=day,
                      S0=S0, burnin=tau_day*2, path=garch_data, M=5000,
                      overwrite=overwrite)
    HD.get_hd()
...
for day in days:
    ...
    for tau in taus:
        ...
        if (tau_day > 1) & (tau_day < 50):
            try:
                fig, filename = plot_MKM(RndData, HdData, day, tau_day, x=x,
                                         reset_S=False, overwrite=True,
                                         h_densfit=0.15)
